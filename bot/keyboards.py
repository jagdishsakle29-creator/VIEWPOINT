...
def get_main_menu_keyboard(bot_username, user_id):
    ...
    keyboard = [
        [
            InlineKeyboardButton("🚀 PLAY VIEWPOINT CASINO (Mini App)", web_app=WebAppInfo(url=game_url))
        ],
        [
...
def get_referral_keyboard(bot_username, user_id):
    ...
    share_text = f"🔥 Play Mines, Crash, Plinko & Limbo on VIEWPOINT Casino and win real cash! Join now using my link: {ref_link}"
